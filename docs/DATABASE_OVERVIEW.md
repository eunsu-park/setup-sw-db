...
|-------|---------|---------|-----------------|
| `omni_low_resolution` | 1 hour | 55 | `kp_index`, `dst_index_nt`, `ap_index_nt`, `f10_7_index_sfu`, proton fluxes |
| `omni_high_resolution` | 1 min | 46 | `flow_speed_km_s`, `bz_gsm_nt`, `sym_h_nt`, `ae_index_nt` |
| `omni_high_resolution_5min` | 5 min | 49 | Same as 1-min + `proton_flux_gt10/30/60mev` (embedded GOES proton) |

All three use `datetime` as a single-column primary key and are updated
per-year (download + delete-by-year + bulk insert).

세 테이블 모두 `datetime` 단일 PK 이며, 연 단위로 다운로드 → delete-by-year
→ bulk insert 로 갱신된다.
...
Source selection in `core/goes.py`:

- GOES-R XRS / MAG / Proton: `data.ngdc.noaa.gov/platforms/solar-space-observing-satellites/goes/`
- Legacy XRS (science-quality): `www.ncei.noaa.gov/data/goes-space-environment-monitor/access/science/xrs/`
